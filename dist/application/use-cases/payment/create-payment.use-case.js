...
let CreatePaymentUseCase = class CreatePaymentUseCase {
    ...
    async execute(doctorId, input) {
        ...
        if (course.patientId !== input.patientId.trim()) {
            throw new validation_error_1.ValidationError('Patient mismatch: Payment.patient must equal TreatmentCourse.patient');
        }
    ...
    validateInput(input) {
        ...
        if (input.amount === undefined || input.amount <= 0) {
            throw new validation_error_1.ValidationError('amount is required and must be greater than zero');
        }
        if (!input.method) {
            throw new validation_error_1.ValidationError('method is required');
        }
        if (input.paidAt && isNaN(new Date(input.paidAt).getTime())) {
            throw new validation_error_1.ValidationError('Invalid paidAt date');
        }
    }
