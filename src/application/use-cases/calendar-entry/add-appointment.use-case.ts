...
import { IAddAppointmentUseCase, AppointmentInput } from '../../interfaces/use-cases/calendar-entry/calendar-entry-use-cases.interface';
...
export class AddAppointmentUseCase implements IAddAppointmentUseCase {
  ...
  async execute(entryId: string, doctorId: string, appointment: AppointmentInput): Promise<void> {
    ...
    if (appointment.startTime && appointment.endTime) {
      for (const existingAppt of entry.appointments) {
    ...
    const updatedAppointments = [...entry.appointments, newAppointment];
