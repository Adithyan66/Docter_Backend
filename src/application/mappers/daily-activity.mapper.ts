import { DailyActivityAggregatedResult } from '../../domain/repositories/visit.repository';
import {
  DailyActivityItemDto,
  DailyActivitySummaryDto,
  PaginatedDailyActivitiesResponseDto,
} from '../../presentation/dto/daily-activity.dto';

export const mapToDailyActivityResponse = (
  result: DailyActivityAggregatedResult
): PaginatedDailyActivitiesResponseDto => {
  const summary: DailyActivitySummaryDto = {
  ...
  const activities: DailyActivityItemDto[] = result.activities.map((activity) => ({
